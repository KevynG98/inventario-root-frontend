...
export const ContextProvider = ({ children }) => {
  ...
  const exportarUsuariosPDF = async () => {
    try {
      const url = `${API_URL}auditoria/usuarios-exportar-pdf/`; // ⚠️ Ajustá la URL real si es distinta
      ...
      a.href = urlBlob;
      a.download = `usuarios_${new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')}.pdf`;
