...
export const ContextProvider = ({ children }) => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const response = await getData(`mantenimiento/cuentas-contables/?${qs.toString()}`);
  ...
  const enviarDatos = async (data) => {
    ...
    try {
      const response = await postData("mantenimiento/cuentas-contables/crear/", data);
      if (response?.status === 201 && response.data) {
        NotificationManager.success("Cuenta Contable creada", "Éxito", 3000);
        showModal()
      } else {
  ...
  const actualizarProveedor = async (datos) => {
    try {
      const response = await putData(`mantenimiento/cuentas-contables/actualizar/${datos.id}/`, datos);

      if (response.status === 200 || response.status === 204) {
        console.log("Cuenta contable actualizada con éxito:", response.data);
        NotificationManager.success("Cuenta Contable editada", "Éxito", 3000);
  ...
  const eliminarProveedor = async (id) => {
    const confirmed = await Swal.fire({
      title: '¿Estás seguro?',
      text: 'Esta acción eliminará permanentemente la cuenta contable.',
      icon: 'warning',
    ...
    if (confirmed.isConfirmed) {
      try {
        const response = await deleteData(`mantenimiento/cuentas-contables/eliminar/${id}/`);
        console.log('Proveedor eliminado:', response.status);
        NotificationManager.success("Cuenta Contable eliminada", "Éxito", 3000);
