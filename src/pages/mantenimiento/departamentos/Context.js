...
export const ContextProvider = ({ children }) => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const response = await getData(`mantenimiento/departamentos/?${qs.toString()}`);
  ...
  const enviarDatos = async (data) => {
    ...
    try {
      const response = await postData("mantenimiento/departamentos/crear/", data);
      if (response?.status === 201 && response.data) {
        NotificationManager.success("Departamento creado", "Éxito", 3000);
        showModal()
      } else {
  ...
  const actualizarProveedor = async (datos) => {
    try {
      const response = await putData(`mantenimiento/departamentos/actualizar/${datos.id}/`, datos);

      if (response.status === 200 || response.status === 204) {
        console.log("Departamento actualizado con éxito:", response.data);
        NotificationManager.success("Departamento editado", "Éxito", 3000);
  ...
  const eliminarProveedor = async (id) => {
    const confirmed = await Swal.fire({
      title: '¿Estás seguro?',
      text: 'Esta acción eliminará permanentemente el departamento.',
      icon: 'warning',
    ...
    if (confirmed.isConfirmed) {
      try {
        const response = await deleteData(`mantenimiento/departamentos/eliminar/${id}/`);
        console.log('Proveedor eliminado:', response.status);
        NotificationManager.success("Departamento eliminado", "Éxito", 3000);
