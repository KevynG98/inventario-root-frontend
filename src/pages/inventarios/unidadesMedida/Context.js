...
export const ContextProvider = ({ children }) => {
  ...
  const enviarDatos = async (data) => {
    ...
    try {
      const response = await postData("inventario/medidas-crear/", data);
  ...
  const actualizarProveedor = async (datos) => {
    try {
      const response = await putData(`inventario/medidas-actualizar/${datos.id}/`, datos);
  ...
  const eliminarProveedor = async (id) => {
    ...
    if (confirmed.isConfirmed) {
      try {
        const response = await deleteData(`inventario/medidas-eliminar/${id}/`);
