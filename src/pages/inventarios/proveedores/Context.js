...
export const ContextProvider = ({ children }) => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      const response = await getData(`inventario/proveedores/?page=${page}&page_size=50`);
  ...
  const enviarDatos = async (data) => {
    ...
    try {
      const response = await postData("inventario/proveedores-crear/", data);
  ...
  const actualizarProveedor = async (datos) => {
    try {
      const response = await putData(`inventario/proveedores-actualizar/${datos.id}/`, datos);

      if (response.status === 200 || response.status === 204) {
        console.log("Proveedor actualizado con éxito:", response.data);
        NotificationManager.success("Proveedor Editado con exito", "Éxito", 3000);
  ...
  const eliminarProveedor = async (id) => {
    const confirmed = await Swal.fire({
      title: '¿Estás seguro?',
      text: 'Esta acción eliminará permanentemente el proveedor.',
      icon: 'warning',
    ...
    if (confirmed.isConfirmed) {
      try {
        const response = await deleteData(`inventario/proveedores-eliminar/${id}/`);
        console.log('Proveedor eliminado:', response.status);
        NotificationManager.success("Proveedor eliminado con éxito", "Éxito", 3000);
